...
class timeletter {
  ...
  async get() {
    console.log('时光信')
    let sql = 'select * from timeletter,users,backimg where timeletter.uid=users.uid and timeletter.backimgid=backimg.backimgid order by timeletter.letterid'
  ...
  async insert(d) {
    ...
    this.mdb.connect().then(client => {
      client.query('insert into timeletter values($1::int,$2::timestamp,$3::int,$4::int,$5::text,$6::int)', [letterid, data.lettertime, data.fontid, data.backimgid, data.lcontent, data.uid]).then(res => {
