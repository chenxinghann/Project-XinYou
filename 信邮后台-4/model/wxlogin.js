...
class userlist {
  ...
  async get(c) {
    let sql = 'select * from users order by uid';
  ...
  async insert(data) {
    ...
    if (!data.uname) {
      ...
      this.mdb.connect().then(client => {
        client.query('insert into users values($1::int,$2::text,$3::text,$4::int,$5::text,$6::text,$7::text,$8::text)', [uid, data.uname, data.uimg, data.age, data.area, data.interest, data.job, data.openid]).then(res => {
